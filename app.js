...
const SCENARIOS = {
  ...
  s4: {
    ...
    observationAmount: 0, observationAvailable: false, lastChecked: null, checkResult: 'no-match', handoffReviewed: false, review: null, notes: [], stale: false, sourceUnavailable: false, positionMismatch: true, correctedSnapshot: false,
...
function statusFor(scenario) {
  ...
  if (scenario.positionMismatch) return { key: 'attention', label: 'Needs attention', copy: scenario.correctedSnapshot ? 'Corrected source snapshot ready to check' : scenario.discrepancy };
...
function nextActionCopy(s, view) {
  if (s.id === 's4' && s.positionMismatch && !s.correctedSnapshot) return `${s.discrepancy} The last validated amount remains ${formatAmount(currentAmount(s))}; no replacement amount is inferred.`;
...
function renderQueue() {
  ...
  return `<section class="page-header"><div><p class="eyebrow">Atlas Exchange / settlements</p><div class="title-row"><h1>Settlement queue</h1><span class="live-pill"><span class="pulse"></span>Northstar only</span></div><p class="page-description">One settlement relationship requires a clear next action.</p></div><div class="header-actions"><button class="button button-primary" type="button" data-action="reset"><span class="button-icon">⟲</span>Reset walkthrough</button></div></section>
    <section class="queue-card single-queue-card" aria-labelledby="queue-title"><div class="section-heading"><div><div class="section-title-row"><h2 id="queue-title">Northstar Institutional</h2><span class="record-count">Trading account · USDC</span></div><p>Review the current amount, readiness, and settlement evidence.</p></div><div class="queue-source"><span class="footer-dot"></span>Source data as of <strong>${s.asOf}</strong></div></div>
      <div class="queue-table-wrap"><table class="queue-table"><thead><tr><th>Relationship</th><th>Amount to settle</th><th>Readiness</th><th>Next action</th><th><span class="sr-only">Actions</span></th></tr></thead><tbody><tr><td><div class="client-cell"><span class="client-avatar">NI</span><span><span class="cell-title">Northstar Institutional</span><span class="cell-subtitle">Atlas / Trading account · USDC</span></span></div></td><td class="amount-cell"><span class="amount ${projection.obligation < 0 ? 'negative' : projection.obligation === 0 ? 'zero' : 'positive'}">${formatAmount(projection.obligation)}</span><span class="subtle-label">${s.positionMismatch && !s.correctedSnapshot ? `Last validated amount · ${s.asOf}` : direction(projection.obligation, 'Northstar')}</span></td><td class="payment-cell"><span class="state-badge ${statusClass}"><span class="badge-dot"></span>${status.label}</span></td><td><span class="subtle-label">${status.copy}</span></td><td><button class="row-action" type="button" data-action="open-row">Open case</button></td></tr></tbody></table></div>
...
function renderChecksCard(s) {
  ...
  const rows = [['✓', '', 'Reconciliation', 'Venue and custody positions reconcile at this source timestamp.', 'Pass', ''], ['✓', '', 'Policy and compliance controls', 'Configured policies and controls pass for this amount.', 'Pass', ''], ['✓', '', 'Settlement-fund availability', 'Sufficient illustrative balance for this amount.', 'Pass', ''], observation];
