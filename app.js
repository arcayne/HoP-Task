...
function renderChecksCard(s) {
  const observation = s.positionMismatch && !s.correctedSnapshot ? ['!', 'warn', 'Position reconciliation', s.discrepancy, 'Review', 'warn'] : s.id === 's4' && s.correctedSnapshot && s.positionMismatch ? ['!', 'warn', 'Position reconciliation', 'Corrected position snapshot awaiting reconciliation.', 'Review', 'warn'] : s.sourceUnavailable ? ['!', 'warn', 'Settlement evidence', 'Check unavailable. The previous amount remains in place.', 'Review', 'warn'] : s.checkResult === 'no-match' ? ['!', 'warn', 'Settlement evidence', 'No matching settlement record is available.', 'Review', 'warn'] : s.checkResult === 'matched' ? ['✓', '', 'Settlement evidence', 'Matching movement observed and applied once.', 'Pass', ''] : ['i', 'neutral', 'Settlement evidence', 'No observed settlement has been applied.', 'Not checked', 'neutral'];
  const reconciliation = s.checkResult === 'changed' ? ['✓', '', 'Position reconciliation', 'Corrected custody snapshot matches expected position.', 'Pass', ''] : s.positionMismatch ? ['✓', '', 'Venue activity calculation', 'Signed venue activity totals 110,000 USDC; custody comparison remains unresolved.', 'Pass', ''] : ['✓', '', 'Position reconciliation', 'Venue and custody positions reconcile at this source timestamp.', 'Pass', ''];
  const rows = [reconciliation, ['✓', '', 'Policy and compliance controls', 'Configured policies and controls pass for this amount.', 'Pass', ''], ['✓', '', 'Settlement-fund availability', 'Sufficient illustrative balance for this amount.', 'Pass', ''], observation].filter(Boolean);
...
function checkAgain() {
  ...
  if (s.id === 's4' && s.positionMismatch && !s.correctedSnapshot) { s.checkResult = 'no-match'; s.history.unshift(['checked', 'Position reconciliation checked; discrepancy remains.', s.discrepancy, nowLabel()]); render(); return showToast('Position reconciliation checked · discrepancy remains'); }
