...
const SCENARIOS = {
  s1: {
    ...
    history: [['proposal', 'Settlement proposal created', '100,000 USDC · controls evaluated', '07 Sep · 09:18 UTC'], ['review', 'Window opened for review', 'Amount to settle confirmed from source records', '07 Sep · 09:00 UTC']]
  },
  s2: {
  ...
  s3: {
    ...
    events: [{ id: 's3-activity-1', type: 'venue', signed: 110000, title: 'Instruction-window activity accrued', detail: '+110,000 USDC · Northstar owes Atlas', time: '07 Sep 2026 · 13:15 UTC' }, { id: 's3-activity-2', type: 'venue', signed: 15000, postCutoff: true, title: 'Post-cutoff activity accrued', detail: '+15,000 USDC · current amount to settle 125,000', time: '07 Sep 2026 · 14:30 UTC' }],
    ...
    cutoff: '07 Sep 2026 · 13:15 UTC', proposalVersion: 'v3 · 13:18 UTC', destination: 'Atlas settlement wallet', sourceAccount: 'Northstar trading account · sim://acct-ns-01',
    history: [['issued', 'Instruction issued for external execution', '110,000 USDC · no matching settlement observed', '07 Sep 2026 · 13:24 UTC'], ['activity', 'Post-cutoff activity accrued', '+15,000 USDC · current amount to settle 125,000', '07 Sep 2026 · 14:30 UTC']]
  }
};
...
const canReview = scenario => !scenario.stale && !scenario.sourceUnavailable && !scenario.instructions.some(instruction => instruction.status !== 'Settled') && (!scenario.replaySteps || replayComplete(scenario)) && currentObligation(scenario) !== 0;
...
function statusFor(scenario) {
  ...
  if (instruction?.status === 'Settled') return obligation === 0 ? { key: 'settled', label: 'Settled', copy: 'Settlement observed · no amount to settle' } : { key: 'residual', label: 'Residual to review', copy: `${formatAmount(obligation)} remains after settlement` };
  if (instruction?.status === 'Awaiting settlement') return { key: 'awaiting', label: 'Settlement not observed', copy: scenario.checkResult === 'no-match' ? 'No matching settlement observed · check again' : 'Issued for custody transfer outside Range' };
  ...
  return { key: 'ready', label: 'Ready for review', copy: 'Safe and permitted to settle' };
}
...
function renderQueue() {
  ...
  const statusClass = status.key === 'awaiting' ? 'blocked' : status.key === 'settled' ? 'confirmed' : status.key === 'residual' ? 'ready' : status.key === 'replay' ? 'ready' : status.key === 'blocked' ? 'blocked' : 'ready';
  ...
  return `<section class="page-header"><div><p class="eyebrow">Atlas Exchange / settlements</p><div class="title-row"><h1>Settlement queue</h1><span class="live-pill"><span class="pulse"></span>Northstar only</span></div><p class="page-description">One settlement relationship requires a clear next action.</p></div><div class="header-actions"><button class="button button-primary" type="button" data-action="reset"><span class="button-icon">⟲</span>Reset walkthrough</button></div></section>
    <section class="queue-card single-queue-card" aria-labelledby="queue-title"><div class="section-heading"><div><div class="section-title-row"><h2 id="queue-title">Northstar Capital</h2><span class="record-count">Trading account · USDC</span></div><p>Review the current amount and whether settlement was observed.</p></div><div class="queue-source"><span class="footer-dot"></span>Source data as of <strong>${s.asOf}</strong></div></div>
      <div class="queue-table-wrap"><table class="queue-table"><thead><tr><th>Relationship</th><th>Amount to settle</th><th>Settlement status</th><th>Next action</th><th><span class="sr-only">Actions</span></th></tr></thead><tbody><tr><td><div class="client-cell"><span class="client-avatar">NC</span><span><span class="cell-title">Northstar Capital</span><span class="cell-subtitle">Atlas / Trading account · USDC</span></span></div></td><td class="amount-cell"><span class="amount ${projection.obligation < 0 ? 'negative' : projection.obligation === 0 ? 'zero' : 'positive'}">${formatAmount(projection.obligation)}</span><span class="subtle-label">${direction(projection.obligation)}</span></td><td class="payment-cell"><span class="state-badge ${statusClass}"><span class="badge-dot"></span>${status.label}</span></td><td><span class="subtle-label">${status.copy}</span></td><td><button class="row-action" type="button" data-action="open-row">${actionLabel}</button></td></tr></tbody></table></div>
    ...
    <section class="queue-note-grid"><article class="principles-card"><div class="card-kicker"><span class="kicker-icon">✓</span>Next action</div><h2>${nextActionTitle(s, 'queue')}</h2><p>${nextActionCopy(s, 'queue')}</p><button class="button button-primary" type="button" data-action="presenter-action">${presenterActionLabel(s, 'queue')}</button></article><article class="attention-card"><div class="card-kicker"><span class="kicker-icon amber">i</span>Pilot boundary</div><h2>Custody transfer outside Range</h2><p>Range creates a fixed, approved settlement instruction. An authorized operator uses the existing custody process; Range later checks the available records for evidence that settlement happened.</p><button class="text-button" type="button" data-action="show-pilot-note">Read pilot note <span>→</span></button></article></section>`;
}
...
function nextActionTitle(s, view) {
  ...
  if (s.id === 's2') return 'Walkthrough complete: external handoff.';
  if (instruction.status === 'Awaiting settlement') return s.observationAvailable ? 'Check the settlement record.' : s.checkResult === 'no-match' ? 'Settlement not observed. Check again when evidence changes.' : 'Check available settlement evidence.';
  if (currentObligation(s) !== 0) return 'Review the remaining amount to settle separately.';
...
function nextActionCopy(s, view) {
  ...
  if (s.id === 's2') return 'One 110,000 USDC settlement instruction is ready for the authorized operator to use in the existing custody process. The earlier windows created no instructions. This walkthrough ends at the handoff.';
...
function presenterActionLabel(s, view) {
  ...
  if (s.id === 's2' && activeInstruction(s)) return 'Walkthrough complete';
  ...
  return canReview(s) ? 'Review remaining amount' : 'Walkthrough complete';
}
...
function renderChecksCard(s, instruction) {
  ...
  const rows = [
    ['✓', '', 'Reconciliation', 'Venue and custody positions reconcile at this source timestamp', 'Pass', ''],
    ['✓', '', 'Policy and compliance controls', 'Configured policies and controls pass for this proposal', 'Pass', ''],
...
function renderGuidanceCard(s, instruction, projection) {
  ...
  return `<article class="detail-card investigation-card"><div class="detail-card-header"><h2>Operator guidance</h2><span class="illustrative">Prototype note</span></div><div class="investigation-body"><p class="investigation-lead">${settled ? 'The matching settlement was observed and applied once. The original instruction is no longer unresolved.' : instruction ? 'The issued amount is frozen while external execution remains unobserved. New activity changes the amount to settle, not the instruction.' : 'Review the exact amount and evidence before creating an instruction for external execution.'}</p><div class="investigation-section"><h3>Next action</h3><p>${nextActionCopy(s, 'case')}</p></div>${settled && residual > 0 ? `<div class="investigation-section"><h3>Residual amount to settle</h3><p>${formatAmount(residual)} remains owed. It requires a separate review and approval; no new instruction is issued automatically.</p></div>` : ''}${s.id === 's2' ? `<div class="investigation-section"><h3>Reversal acceptance case</h3><p>Submit 100,000 USDC, then apply 20,000 USDC of Northstar earnings. The instruction remains 100,000 while the amount to settle becomes 80,000; if it settles, Atlas owes Northstar 20,000. No automatic reverse instruction is created.</p></div>` : ''}</div></article>`;
}
...
function submitMock() {
  ...
  const instruction = { id: `sim-instruction-${s.id}-${s.instructions.length + 1}`, amount, status: 'Awaiting settlement', issuedAt: s.asOf, cutoff: s.cutoff, proposalVersion: s.proposalVersion, sourceAccount: s.sourceAccount, destination: `${s.destination} · sim://acct-atlas-01` };
  s.observationAvailable = false; s.checkResult = null; s.lastChecked = null; s.escalated = false;
  ...
  closeModal(); state.view = 'case'; render(); showToast('Instruction created · use the existing custody process');
}
...
function checkAgain() {
  ...
  if (s.observationAvailable) {
    ...
    instruction.status = 'Settled'; s.checkResult = 'matched';
    if (currentObligation(s) !== 0) { s.cutoff = s.asOf; s.proposalVersion = `v${s.instructions.length + 3} · after settlement observation`; } s.history.unshift(['settled', 'Matching settlement observed', `${formatAmount(instruction.amount)} applied once`, nowLabel()]);
...
function showPilotNote() { openModal(`<div class="modal-header"><div><h2 id="modal-title">Pilot boundary</h2><p>Custody transfer outside Range is an explicit trade-off for this prototype.</p></div><button class="modal-close" type="button" data-action="close-modal" aria-label="Close dialog">×</button></div><div class="modal-body"><p class="modal-note" style="margin:0">Range calculates the amount to settle, reconciles venue and custody positions, checks configured policies and controls, obtains Atlas approval, and creates a fixed settlement instruction. An authorized operator uses Fireblocks, Copper, or the applicable existing custody process. Range then checks available records to determine whether settlement happened. The exact actor, permissions, and reference propagation need validation with the Range team.</p></div><div class="modal-footer"><button class="button button-primary" type="button" data-action="close-modal">Done</button></div>`); }
...
function openModal(html) { state.modalReturnFocus = document.activeElement; $('#modal').innerHTML = html.replaceAll('Current obligation', 'Amount to settle').replaceAll('current obligation', 'current amount to settle').replaceAll('Manual custody execution', 'Custody transfer outside Range').replaceAll('fund availability', 'policy and compliance controls'); $('#modal-backdrop').hidden = false; $('#modal').hidden = false; setTimeout(() => $('.modal-close')?.focus(), 0); }
...
function walkthroughProgress(s, view) {
  const instruction = activeInstruction(s);
  if (s.id === 's2') return { step: instruction ? 7 : s.replayIndex + 1, total: 7 };
  const originalS3 = s.id === 's3' && s.instructions.length === 1;
  const total = originalS3 ? 5 : 6;
  if (instruction?.status === 'Settled') return { step: total, total };
  if (!instruction) return { step: view === 'queue' ? 1 : 2, total };
  if (s.observationAvailable) return { step: total - 1, total };
  if (s.checkResult === 'no-match') return { step: total - 2, total };
  return { step: originalS3 ? (view === 'queue' ? 1 : 2) : 3, total };
}
function renderPresenter() {
  const s = state.scenario; const { step, total } = walkthroughProgress(s, state.view);
  const action = $('#presenter-action'); const label = presenterActionLabel(s, state.view); const complete = label === 'Walkthrough complete';
...
function normalizeVisibleCopy() {
  ...
  root.innerHTML = root.innerHTML
    .replaceAll('Current obligation', 'Amount to settle')
    .replaceAll('current obligation', 'current amount to settle')
    .replaceAll('Manual custody execution', 'Custody transfer outside Range')
    .replaceAll('Execute in your custody tool', 'Custody transfer outside Range')
    .replaceAll('Awaiting settlement', 'Settlement not observed')
    .replaceAll('Controls available for this proposal', 'Safe and permitted to settle');
  root.querySelectorAll('.unknown-banner').forEach(banner => {
    if (banner.textContent.includes('Settlement not observed')) banner.innerHTML = '<span></span>Instruction issued';
  });
  const queueNextAction = root.querySelector('.queue-table tbody td:nth-child(4)');
  if (queueNextAction) queueNextAction.innerHTML = `<span class="subtle-label">${presenterActionLabel(state.scenario, 'queue')}</span>`;
}
function render() { if (state.view === 'case') renderCase(); else renderQueueView(); normalizeVisibleCopy(); renderPresenter(); }
function navigate(view) { state.view = view; render(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
function handlePresenterAction() { const s = state.scenario; if (s.id === 's2' && !replayComplete(s)) { s.replayIndex += 1; render(); showToast(`${s.replaySteps[s.replayIndex].title} · ${formatAmount(currentObligation(s))} amount to settle`); return; } if (s.id === 's2' && activeInstruction(s)) return; if (state.view === 'queue') { navigate('case'); return; } const instruction = activeInstruction(s); if (canReview(s)) { openReview(); return; } if (instruction.status === 'Awaiting settlement') { if (s.observationAvailable) checkAgain(); else if (s.checkResult === 'no-match') simulateObservation(); else checkAgain(); } }
